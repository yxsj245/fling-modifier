...
def download_and_extract_attachment(url):
    print("INFO:正在访问和解析网站，如果长时间无反应后崩溃则是网络或网站问题")
    ...
    if first_attachment_url:
        ...
        print(f"INFO:成功解析出下载地址，文件名称: {filename}")
        ...
        if not files:
            ...
            for file in files:
                xuanze = input('interaction:检测到修改器已下载过，是否删除之前下载的(y表示确认删除,输入任意内容表示不删除)')
                if xuanze == 'y':
                    files = os.listdir(directory_path)
                    files = files[0]
                    # print(files)
                    ...
                    print('WARN:用户已取消删除，这可能会导致使用此程序启动修改器为之前下载的')

        # 下载文件
        print(f"INFO:开始下载修改器")
        if not os.path.exists(folder_path) or filename not in os.listdir(folder_path):
            ...
            if filepath:
                ...
                print("INFO:下载完毕，请重新运行程序即可启动修改器界面选择对应修改器即可启动")
        else:
            print("ERROR:目标目录中已存在文件。可能是解压文件失败。")
...
def launch_exe(directory):
    ...
    if exes:
        ...
        print("ERROR:在目录中找不到可执行文件(.exe);可能是您手动删除")
...
if data is not None and not data["run"]:
    # 调用函数并传入指定网站的URL
    urldata = input("interaction:请输入需要下载的风灵月影修改器地址:")
    url = urldata
    download_and_extract_attachment(url)
    print("INFO:程序运行完毕，按任意键退出...")
    input()
else:
    #启动修改器主程序
    data_dir = 'data'
    ...
    try:
        print("WARN:权限提升安全提醒：由于Windows权限规则，启动exe文件必须以管理员身份运行，输入后程序将会请求Windows以管理员身份启动修改器。")
        choice = int(input("interaction:请输入要启动的游戏名称修改器(输入前面序号即可): "))
